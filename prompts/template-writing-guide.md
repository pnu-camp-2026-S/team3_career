...
현재는 `lib/analysis/templates.mjs` 렌더러가 이 규칙대로 결정적으로(비 AI) 채우고,
...
3. **enum 준수** — 하위 폴더는 `lib/analysis/subfolder-config.mjs`의 enum(presentation, report, data, code, planning, submission, visual, pending, other)만 사용한다.
