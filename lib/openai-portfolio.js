      ...
      items: {
        ...
        properties: {
          ...
          actions: {
            type: 'OBJECT',
            properties: {
              problemAction: stringArray,
              productivityAction: stringArray,
              communicationAction: stringArray,
            },
          },
        },
      },
...
function normalizePpt(data) {
  if (Array.isArray(data.experiences) && data.experiences.length) {
    ...
    const slides = [
      {
        title: cover.headline || data.deckTitle || 'PPT 발표 스펙',
        body: stringifyList([
          cover.description,
          cover.name && `Name: ${cover.name}`,
          (cover.mobile || cover.phone) && `Mobile: ${cover.mobile || cover.phone}`,
          cover.email && `E-mail: ${cover.email}`,
          cover.web && `Web: ${cover.web}`,
        ]),
      },
      {
        title: profile.greeting || '프로필',
        body: stringifyList([
          profile.intro,
          profile.about?.length && `About: ${profile.about.join(', ')}`,
          profile.education?.length && `Education: ${profile.education.map((item) => `${item.period} ${item.content}`).join(' / ')}`,
          profile.experience?.length && `Experience: ${profile.experience.map((item) => `${item.period} ${item.content}`).join(' / ')}`,
          profile.licenses?.length && `Licenses: ${profile.licenses.map((item) => `${item.period} ${item.content}`).join(' / ')}`,
          profile.skills?.length && `Skills: ${profile.skills.map((item) => `${item.name} ${item.level || ''}`.trim()).join(', ')}`,
        ]),
      },
      ...data.experiences.flatMap((item, index) => {
        const cards = item.cards || {};
        const actions = item.actions || {};
        return [
          {
            title: item.introTitle || `경험 ${index + 1} 소개`,
            body: stringifyList([
              cards.customerNeed?.description && `고객 니즈: ${cards.customerNeed.description}`,
              cards.problemOpportunity?.description && `문제 OR 기회상황: ${cards.problemOpportunity.description}`,
              cards.comparisonTarget?.description && `비교 대상: ${cards.comparisonTarget.description}`,
            ]),
          },
          {
            title: item.resultTitle || `경험 ${index + 1} 결과/KPI`,
            body: stringifyList([
              item.visual?.title && `시각 자료: ${item.visual.title}`,
              ...(item.visual?.items || []),
              actions.problemAction?.length && `문제 해결 액션: ${actions.problemAction.join(', ')}`,
              actions.productivityAction?.length && `생산성 증가 액션: ${actions.productivityAction.join(', ')}`,
              actions.communicationAction?.length && `의사소통/협상 액션: ${actions.communicationAction.join(', ')}`,
            ]),
          },
        ];
      }),
    ...
    return buildNormalized(
      cover.headline || data.deckTitle || 'PPT 발표 스펙',
      cover.description || 'PPT 발표 흐름에 맞춰 구성한 초안입니다.',
      slides,
      slides,
      data
    );
...
function buildMockPptData(context) {
  ...
  return {
    ...
    deckTitle: 'PPT 발표 스펙',
    cover: {
      headline: `성장하고 확장하는 ${context.myPageInfo?.name || '지원자'}입니다.`,
      description: `${mainExperience}를 중심으로 핵심 경험과 결과를 정리한 발표용 포트폴리오입니다.`,
      name: context.myPageInfo?.name || '목데이터 사용자',
      mobile: context.myPageInfo?.phone || '',
      email: context.myPageInfo?.email || '',
      web: context.myPageInfo?.github || '',
    },
    profile: {
      photo: '',
      greeting: `안녕하세요\n${context.myPageInfo?.name || '지원자'}입니다.`,
      about: [
        context.myPageInfo?.name || '목데이터 사용자',
        context.myPageInfo?.birth || '',
        context.myPageInfo?.email || '',
        context.myPageInfo?.phone || '',
      ].filter(Boolean),
      intro: `${context.major} 기반으로 ${keywords.slice(0, 2).join(', ')} 역량을 쌓아온 지원자입니다.`,
      education: [
        { period: '재학 중', content: `${context.myPageInfo?.school || '부산대학교'} ${context.major}` },
      ],
      experience: [
        { period: '2026', content: mainExperience },
      ],
      licenses: [],
      skills: keywords.map((keyword) => ({ name: keyword, level: '핵심 역량' })),
    },
    experiences: [
      {
        projectLabel: 'Project 01',
        introTitle: `경험 1 : ${mainExperience}에서 보여준 ${keywords[0]} 역량`,
        cards: {
          customerNeed: {
            image: '',
            description: '사용자가 가진 자료를 발표 가능한 경험으로 정리할 필요가 있었습니다.',
            keyword: keywords[0] || '문제 해결',
          },
          problemOpportunity: {
            image: '',
            description: '자료는 있었지만 핵심 메시지와 결과 흐름이 분리되어 있었습니다.',
            keyword: keywords[1] || '자료 구조화',
          },
          comparisonTarget: {
            image: '',
            description: '기존 정리 방식과 발표용 포트폴리오 구성을 비교했습니다.',
            keyword: keywords[2] || '결과 정리',
          },
        },
        resultTitle: '결과물 : 경험의 흐름을 발표 가능한 구조로 정리',
        visual: {
          type: 'qualitative_card',
          title: mainExperience,
          items: ['핵심 경험 정리', '발표 흐름 구성', '보완 정보 확인'],
        },
        actions: {
          problemAction: ['흩어진 자료를 경험 단위로 묶음', '핵심 문제와 목표를 분리해 정리'],
          productivityAction: ['AI 초안을 바탕으로 발표 구조를 빠르게 검토'],
          communicationAction: ['발표자가 말로 보충할 수 있도록 문장을 압축'],
        },
      },
    ],
