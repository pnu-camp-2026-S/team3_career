...
function normalizeStringArray(items, limit = 3) {
...
function normalizeDetail(data, context = {}) {
  if (data?.templateId === 'detail_a4_two_page_half_blocks' || data?.profile || Array.isArray(data?.experiences)) {
    const normalized = normalizeDetailA4Data(data, context);
    const profile = normalized.profile || {};
    const keywordLabels = (profile.keywords || []).map((item) => item.label).filter(Boolean);
    const blocks = [
      {
        title: '사용자 개요',
        body: stringifyList([
          profile.name && `이름: ${profile.name}`,
          profile.education && `학력: ${profile.education}`,
          profile.major && `전공: ${profile.major}`,
          profile.targetRole && `희망 직무: ${profile.targetRole}`,
          keywordLabels.length && `강조 키워드: ${keywordLabels.join(', ')}`,
        ]),
      },
      ...normalized.experiences.map((experience) => ({
        title: `경험 ${experience.rank}: ${experience.projectName}`,
        body: stringifyList([
          experience.summary,
          experience.process.length && `과정: ${experience.process.join(' / ')}`,
          experience.contribution.length && `기여도: ${experience.contribution.join(' / ')}`,
          experience.result.length && `결과: ${experience.result.join(' / ')}`,
          experience.growth && `발전한 점: ${experience.growth}`,
        ]),
      })),
    ].filter((block) => block.body);

    const firstExperience = normalized.experiences[0];
    return buildNormalized(
      `${profile.name || context.myPageInfo?.name || '지원자'} 상세 기술 포트폴리오`,
      firstExperience?.summary || 'A4 2장 상세기술 포트폴리오 초안입니다.',
      blocks,
      blocks,
      normalized
    );
...
function normalizeDetailA4Data(data, context = {}) {
  const info = context.myPageInfo || {};
  const profile = data.profile || {};
  const keywords = normalizeDetailKeywords(profile.keywords, context.keywords);
  const experiences = normalizeDetailExperiences(data.experiences, context);
  ...
  return {
    ...
    profile: {
      photo: {
        path: compactSentence(profile.photo?.path || info.photo || info.photoUrl),
        description: compactSentence(profile.photo?.description || '프로필 사진 없음'),
      },
      name: compactSentence(profile.name || info.name || '정보 없음'),
      gender: compactSentence(profile.gender || info.gender || '정보 없음'),
      education: compactSentence(profile.education || summarizeEducation(info) || '정보 없음'),
      major: compactSentence(profile.major || context.major || '정보 없음'),
      targetRole: compactSentence(profile.targetRole || info.targetRole || context.purpose || '정보 없음'),
      contact: compactSentence(profile.contact || info.email || info.phone || '정보 없음'),
      keywords,
    },
    experiences,
    missingFields: normalizeStringArray(data.missingFields || data.missing_fields, 12),
  };
}

function summarizeEducation(info) {
  if (typeof info.education === 'string') return info.education;
  const educations = Array.isArray(info.educations) ? info.educations : [];
  const first = educations[0] || {};
  return [first.school, first.major, first.status || first.grade].filter(Boolean).join(' ');
}

function normalizeDetailKeywords(items, fallbackKeywords = []) {
  const source = Array.isArray(items) && items.length
    ? items
    : (fallbackKeywords || []).slice(0, 3).map((label, index) => ({
      rank: index + 1,
      label,
      reason: '사용자가 선택한 강조 키워드입니다.',
    }));

  const normalized = source
    .map((item, index) => ({
      rank: Number(item?.rank || index + 1),
      label: compactSentence(item?.label || item),
      reason: compactSentence(item?.reason || '선택한 자료에서 확인된 강조 키워드입니다.'),
    }))
  ...
  return normalized.map((item, index) => ({ ...item, rank: index + 1 }));
}

function normalizeDetailExperiences(items, context = {}) {
  const source = Array.isArray(items) && items.length
    ? items
    : (context.experienceProjects || []).map((project, index) => ({
      rank: index + 1,
      projectName: project.projectName,
      summary: project.headline || project.description,
      process: ['자료 범위 확인', '핵심 내용 정리', '포트폴리오 구조화'],
      contribution: ['업로드 자료 정리', 'AI 요약 결과 검토', '보완 포인트 도출'],
      result: ['프로젝트 흐름 정리', '핵심 역량 도출', '추가 보완 항목 확인'],
      growth: '자료 기반으로 경험을 구조화하는 역량을 키웠습니다.',
    }));

  const normalized = source.slice(0, 3).map((item, index) => normalizeDetailExperience(item, index + 1));
  while (normalized.length < 3) {
    normalized.push(buildMissingDetailExperience(normalized.length + 1));
  }
  return normalized;
}

function normalizeDetailExperience(item, rank) {
  return {
    rank: Number(item?.rank || rank),
    projectName: compactSentence(item?.projectName || item?.project_name || '추가 경험 입력 필요'),
    summary: compactSentence(item?.summary || item?.description || '프로젝트 한 줄 설명 보완 필요'),
    process: normalizeStringArray(item?.process, 3),
    contribution: normalizeStringArray(item?.contribution || item?.myContribution, 3),
    result: normalizeStringArray(item?.result || item?.outputs, 3),
    growth: compactSentence(item?.growth || item?.learning || '이 프로젝트를 통해 발전한 점 보완 필요'),
  };
}

function buildMissingDetailExperience(rank) {
  return {
    rank,
...
// 선택한 폴더의 파일별 요약(file_analyses.summary_md)을 프롬프트 텍스트로 변환한다.
// 입력 형태: [{ projectId, label, files: [{ name, summary }] }]
function buildFolderFileSummaryPrompt(folders) {
  ...
  const sections = list
    ...
    .map((folder) => {
      const files = (Array.isArray(folder?.files) ? folder.files : [])
        .slice(0, 15)
        .map((file) => {
          const name = String(file?.name || '이름 없는 파일');
          const summary = compactPromptText(file?.summary || '', 1500);
          return `- ${name}: ${summary || '(요약 없음)'}`;
        });
      if (!files.length) return '';
      const label = String(folder?.label || folder?.projectId || '이름 없는 프로젝트');
      return `■ 프로젝트: ${label}\n${files.join('\n')}`;
    })
    .filter(Boolean);
...
[선택 폴더 파일 요약]
${buildFolderFileSummaryPrompt(folderFileSummaries)}

[근거 사용 규칙]
1. 선택 프로젝트 분석 summary JSON과 선택 폴더 파일 요약을 가장 우선 근거로 사용하세요.
2. 위 요약에 없는 역할, 성과, 수치, 기술 사용 경험은 새로 만들지 마세요.
3. 정보가 부족하면 형식별 스키마의 부족 정보 필드(missingFields 또는 missing_fields)에 보완할 정보를 적으세요.
`;
}
...
function buildMockDetailData(context) {
  ...
  const mainKeyword = firstValue(context.keywords, '문제 해결');
  const keywordSource = context.keywords?.length ? context.keywords : [mainKeyword, '자료 구조화', '문서화'];
  ...
  return {
    ...
    pageCount: 2,
    profile: {
      photo: { path: '', description: '프로필 사진 없음' },
      name: context.myPageInfo?.name || '목데이터 사용자',
      gender: context.myPageInfo?.gender || '정보 없음',
      education: summarizeEducation(context.myPageInfo || {}) || '부산대학교 재학',
      major: context.major,
      targetRole: context.purpose || '희망 직무',
      contact: context.myPageInfo?.email || context.myPageInfo?.phone || '정보 없음',
      keywords: keywordSource.slice(0, 3).map((keyword, index) => ({
        rank: index + 1,
        label: keyword,
        reason: '선택한 프로젝트 요약과 강조 키워드에서 확인된 역량입니다.',
      })),
    },
    experiences: [
      {
        rank: 1,
        projectName: mainExperience,
        summary: `${mainKeyword} 역량이 드러나는 대표 프로젝트 경험입니다.`,
        process: ['업로드 자료 범위 확인', '핵심 키워드 선별', '포트폴리오 흐름 재배치'],
        contribution: ['자료를 경험 단위로 정리', 'AI 요약 결과 확인', '부족한 설명 포인트 도출'],
        result: ['선택 경험의 문제 해결 흐름 정리', '핵심 역량을 한눈에 확인', '정량 성과는 추가 입력 필요'],
        growth: '근거 자료를 바탕으로 경험을 구조화하는 역량을 키웠습니다.',
      },
      {
        rank: 2,
        projectName: context.experiences?.[1] || '추가 경험 입력 필요',
        summary: context.experiences?.[1] ? '두 번째 선택 경험을 상세기술형 구조로 정리했습니다.' : '두 번째 경험 정보가 부족합니다.',
        process: ['경험 자료 확인', '진행 흐름 정리', '보완 항목 분류'],
        contribution: ['본인 역할 확인', '수행 내용 정리', '직무 연결점 도출'],
        result: ['산출물 확인', '배운 점 정리', '추가 보완 필요'],
        growth: '경험을 과정, 기여도, 결과로 나누어 설명하는 관점을 키웠습니다.',
      },
      {
        rank: 3,
        projectName: context.experiences?.[2] || '추가 경험 입력 필요',
        summary: context.experiences?.[2] ? '세 번째 선택 경험을 상세기술형 구조로 정리했습니다.' : '세 번째 경험 정보가 부족합니다.',
        process: ['경험 자료 확인', '진행 흐름 정리', '보완 항목 분류'],
        contribution: ['본인 역할 확인', '수행 내용 정리', '직무 연결점 도출'],
        result: ['산출물 확인', '배운 점 정리', '추가 보완 필요'],
        growth: '여러 경험을 같은 기준으로 정리하는 역량을 키웠습니다.',
      },
    ],
    missingFields: ['구체적 수치', '프로필 사진', '사용 기술 상세'],
  };
}
...
export function buildMockPortfolioResponse(format, context) {
  ...
  if (format.includes('1페이지')) rawData = buildMockOnePageData(context);
