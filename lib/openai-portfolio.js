...
import {
  buildMockCoverLetterV2Data,
  buildPortfolioCoverLetterV2Prompt,
  buildPortfolioCoverLetterV2UserPrompt,
  normalizePortfolioCoverLetterV2,
  portfolioCoverLetterV2Schema,
} from './portfolio-coverletter-v2.js';
...
`,
  ...
  '자기소개서 연결형': {
    schema: portfolioCoverLetterV2Schema,
    prompt: `
[역할 부여]
...
`,
    normalize: normalizePortfolioCoverLetterV2,
  },
};
...
formatPrompts['자기소개서 연결형'].prompt = buildPortfolioCoverLetterV2Prompt();
...
function normalizeCoverLetter(data) {
  if (data?.motivation || data?.representativeExperience || data?.questionMap) {
    const competencyLines = (data.competencies || []).map((item) => [
      item.name && `역량: ${item.name}`,
    ...
    const blocks = [
      {
        title: data.title || '자기소개서형 포트폴리오',
        body: stringifyList([
          data.headline,
          data.coverSummary,
          data.coverChips?.length && `핵심 키워드: ${data.coverChips.join(', ')}`,
        ]),
      },
      {
        title: '지원 동기',
        body: stringifyList([
          data.motivation?.subtitle,
          data.motivation?.narrative,
          ...(data.motivation?.evidence || []).map((item) => `${item.label}: ${item.text}`),
        ]),
      },
      {
        title: '직무 적합 역량',
        body: stringifyList(competencyLines),
      },
      {
        title: data.representativeExperience?.title || '대표 경험 서술',
        body: stringifyList([
          data.representativeExperience?.narrative,
          ...(data.representativeExperience?.star || []).map((item) => `${item.label}: ${item.text}`),
        ]),
      },
      {
        title: '문제 해결 및 협업 방식',
        body: stringifyList([
          ...(data.workStyle?.process || []).map((item) => `${item.label}: ${item.text}`),
          ...(data.workStyle?.collaboration || []).map((item) => `${item.label}: ${item.text}`),
        ]),
      },
      {
        title: '입사 후 기여 계획',
        body: stringifyList((data.contributionPlan || []).map((item) => `${item.period}: ${item.plan} (${item.evidence})`)),
      },
      {
        title: '자기소개서 문항 연결',
        body: stringifyList((data.questionMap || []).map((item) => `${item.question}: ${item.experience} - ${item.answerPoint}`)),
      },
      {
        title: '보완 필요 정보',
        body: stringifyList(data.missingFields || data.missing_fields || []),
      },
    ].filter((block) => block.body);

    const normalizedRaw = {
      ...data,
      templateId: data.templateId || 'coverletter_ppt_v1',
      format: data.format || '자기소개서형 포트폴리오',
      missingFields: data.missingFields || data.missing_fields || [],
    };

    return buildNormalized(
      data.title || '자기소개서형 포트폴리오',
      data.headline || data.coverSummary || '자기소개서 문항 흐름에 맞춘 PPT 초안입니다.',
      blocks,
      blocks,
      normalizedRaw
    );
...
function resolvePortfolioFormat(format) {
  const text = String(format || '').trim();
  if (text.includes('1페이지')) return '1페이지 요약본';
  if (text.includes('자기소개')) return '자기소개서 연결형';
  if (text.includes('PPT')) return 'PPT 발표 스펙';
  return text;
}

export function getPortfolioTemplate(format) {
  const resolvedFormat = resolvePortfolioFormat(format);
  return formatPrompts[resolvedFormat] || formatPrompts['상세 기술 포트폴리오'];
}
...
${resolvedFormat === '1페이지 요약본' ? buildPortfolioOnePageUserPrompt({
    format,
    purpose,
    major,
    experiences,
...
${resolvedFormat === '자기소개서 연결형' ? buildPortfolioCoverLetterV2UserPrompt({
    format,
    purpose,
    major,
    experiences,
...
function buildMockCoverLetterData(context) {
  ...
  const mainKeyword = firstValue(context.keywords, '문제 해결');
  const name = context.myPageInfo?.name || '지원자';
  const role = context.purpose || '지원 직무';

  return {
    templateId: 'coverletter_ppt_v1',
    format: '자기소개서형 포트폴리오',
    title: `${name} | ${role}`,
    headline: `${mainExperience}을 바탕으로 ${mainKeyword} 역량을 보여주는 지원자`,
    applicant: {
      name,
      ...
      email: context.myPageInfo?.email || '',
      phone: context.myPageInfo?.phone || '',
      contactLine: [context.myPageInfo?.email, context.myPageInfo?.phone].filter(Boolean).join(' · '),
    },
    coverChips: [context.major, mainKeyword, mainExperience, `${context.experiences?.length || 1}개 경험`].filter(Boolean).slice(0, 5),
    coverSummary: `${mainExperience}에서 확인한 행동과 배운 점을 자기소개서 문항 흐름으로 정리했습니다.`,
    motivation: {
      subtitle: '선택 경험을 기반으로 한 지원 방향',
      narrative: `${mainExperience}을 통해 ${mainKeyword}의 필요성을 직접 확인했고, 이를 ${role}에서 활용할 수 있는 강점으로 정리했습니다.`,
      evidence: [
        { label: '대표 경험', text: mainExperience },
        { label: '강조 역량', text: mainKeyword },
      ],
    },
    competencies: [
      {
        name: mainKeyword,
        evidence: `${mainExperience}에서 문제를 파악하고 해결 방향을 정리했습니다.`,
        lesson: '경험을 직무 언어로 설명하는 방법을 배웠습니다.',
      },
      {
        name: '자료 구조화',
        evidence: '업로드 자료와 분석 요약을 근거 단위로 정리했습니다.',
        lesson: '근거 기반 포트폴리오 작성의 중요성을 확인했습니다.',
      },
      {
        name: '전공 기반 이해',
        evidence: `${context.major} 전공 정보를 경험 해석의 배경으로 활용했습니다.`,
        lesson: '전공과 경험을 함께 연결해야 설득력이 높아집니다.',
      },
    ],
    representativeExperience: {
      title: mainExperience,
      narrative: `${mainExperience}을 수행하며 상황을 파악하고 필요한 자료를 정리했습니다. 이 과정에서 ${mainKeyword} 역량을 중심으로 나의 행동과 배운 점을 설명할 수 있는 근거를 만들었습니다.`,
      star: [
        { label: '문제', text: '경험의 목적과 전달할 강점이 분산되어 있었습니다.' },
        { label: '행동', text: '자료를 정리하고 핵심 역량과 연결되는 행동을 추렸습니다.' },
        { label: '결과', text: '자기소개서 문항에 연결할 수 있는 경험 흐름을 만들었습니다.' },
      ],
    },
    workStyle: {
      process: [
        { label: '문제 이해', text: '선택 경험에서 드러나는 문제와 목표를 먼저 파악했습니다.' },
        { label: '근거 정리', text: '자료 요약과 키워드를 바탕으로 사용할 근거를 좁혔습니다.' },
        { label: '문장화', text: '경험을 지원 동기와 직무 역량 문장으로 바꿨습니다.' },
      ],
      collaboration: [
        { label: '확인 중심', text: '확인되지 않은 성과나 역할은 보완 정보로 남겼습니다.' },
      ],
    },
    contributionPlan: [
      { period: '30일', plan: '직무와 조직의 일하는 방식을 빠르게 이해합니다.', evidence: `${context.major} 전공 기반 이해` },
      { period: '60일', plan: '선택 경험에서 익힌 문제 해결 방식을 실무 과제에 적용합니다.', evidence: mainExperience },
      { period: '90일', plan: '데이터와 사용자 관점으로 개선 아이디어를 제안합니다.', evidence: mainKeyword },
    ],
    questionMap: [
      { question: '지원 동기', experience: mainExperience, answerPoint: `${mainKeyword}을 직무 관심으로 연결` },
      { question: '직무 역량', experience: mainExperience, answerPoint: '문제 파악과 실행 과정 강조' },
      { question: '입사 후 포부', experience: context.major, answerPoint: '전공 이해와 경험 기반 기여 계획' },
    ],
    missingFields: ['지원 회사명', '직무명', '정량 성과'],
  };
}
...
export function buildMockPortfolioResponse(format, context) {
  ...
  const resolvedFormat = resolvePortfolioFormat(format);
  if (resolvedFormat === '1페이지 요약본') rawData = buildMockPortfolioOnePageData(context);
  if (resolvedFormat === 'PPT 발표 스펙') rawData = buildMockPptData(context);
  if (resolvedFormat === '자기소개서 연결형') rawData = buildMockCoverLetterV2Data(context);
