...
`app/api/signup/route.js`에 `bcryptjs` 기반 `/api/signup` 스캐폴드가 존재하지만, 메모리 배열(`usersDB`)만 사용하는 미완성 상태이며 현재 `signup.html`은 소셜 로그인 버튼만 사용하고 이 엔드포인트를 호출하지 않는다. 실제 이메일 가입은 이후 Supabase Auth와 연결해 구현한다.
